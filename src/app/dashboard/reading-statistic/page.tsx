...
const ReadingStatisticPage = async () => {
    ...
    try {
        const topic_response = await fetch(`${process.env.NEXT_PUBLIC_READING_SERVICE_URL}/api/reading/user/topic-list`, {
            method: 'GET',
            headers: {
        ...
        const summary_response = await fetch(`${process.env.NEXT_PUBLIC_READING_SERVICE_URL}/api/reading/user/summary`, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${session.access_token}`,
        ...
        const tpi_response = await fetch(`${process.env.NEXT_PUBLIC_READING_SERVICE_URL}/api/reading/user/tpi`, {
            method:'POST',
            headers:{
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${session.access_token}`,
        ...
        const history_response = await  fetch(`${process.env.NEXT_PUBLIC_READING_SERVICE_URL}/api/reading/user/answer`, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${session.access_token}`,
        ...
        return <Card.Root shadow={'md'} w={'100%'} p={'2.5%'}>
            <StatisticComponent topics={topic_data} summary={summary_data} tpi={tpi_data} history={history_data} skill={'Reading'} />
        </Card.Root>
    } catch (e) {
        return (<AbsoluteCenter>
            Internal Error
